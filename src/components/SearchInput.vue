  ...
  methods: {
    SearchAction: _.throttle(async function (e) {
      ...
      if (reg.test(inputValue)) {
        ...
        let arr1 = await getLedger({ seq: inputValue }).then((res) => {
        ...
        if (n > 0 && n < 9223372036854775807 && arr1.length !== 0)
          that.$router.push({
            name: "blockdetail",
            params: { block: inputValue },
          });
        else that.$router.push({ path: "/404" });
      } else {

        // let arr2 =  getLedger({ hash: inputValue }).then((res) => {
        //   return res === null ? null : res.result;
        // });
        // let arr3 =  getTransaction({ hash: inputValue }).then((res) => {
        //   return res === null ? null : res.result;
        // });
        // let arr4 =  getAccount({ address: inputValue }).then((res) => {
        //   return res === null ? null : res.result;
        // });
